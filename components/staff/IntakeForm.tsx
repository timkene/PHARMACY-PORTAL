...
interface IntakeFormProps {
  onSubmit: (data: { enrollee: Enrollee; diagnosis: string; medications: Medication[] }) => void
  submitting: boolean
}

const EMPTY_MED: Medication = { name: '', dosage: '', quantity: 1 }
...
export function IntakeForm({ onSubmit, submitting }: IntakeFormProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setMedError('')
    onSubmit({ enrollee, diagnosis, medications: filledMeds })
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="grid grid-cols-2 gap-6 mb-6">
        ...
        {/* Right — Diagnosis & Medications */}
        <div className="bg-surface-lowest border border-outline-variant rounded p-6 space-y-4">
          <h2 className="text-title-md font-semibold text-on-surface">Diagnosis & Medications</h2>

          <div>
            <label className="block text-body-sm font-semibold mb-1">Primary Diagnosis / ICD-10</label>
            <input
              required
              className={INPUT_CLS}
              placeholder="Diabetes mellitus (E11)"
              value={diagnosis}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-body-sm font-semibold">Medications</label>
              <button
                type="button"
                onClick={addMed}
            ...
            {medications.length === 0 && (
              <p className="text-body-sm text-on-surface-variant italic">No medications added yet.</p>
            )}

            {medications.map((med, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_1fr_80px_28px] gap-2 mb-2 items-start">
                <input
                  className={INPUT_CLS}
                ...
                <button
                  ...
                  className="mt-2 text-error hover:text-error/70 text-lg leading-none"
                >
                  ×
                </button>
              </div>
